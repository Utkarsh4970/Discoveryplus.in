...
function Free() {
    ...
    return (
        <Toolbar>
            ...
            <h1 className='pt-3' style={{ fontSize: "20px", color: "white" }}>Your Favourites, Ippothu Tamilil<Link to="/showall" className='float-end pt-0 hov' style={{ color: "#838991", fontSize: "16px", textDecoration: "none" }}>View All</Link></h1>
                <div className="row">
                    {
                        newly.map((e) => (
                            <div className="col col-6  g-3 col-lg-4 col-xl-3" key={e._id} >
                                <Link to="/watch">
                                    <div className="card" key={e._id} style={{ borderRadius: "10px" ,border:"none"}} onClick={()=>{
                                        ...
                                        {
                                            !e.newstatus ? <button className="btn-primary bottomCentered px-2" style={{ fontSize: "14px", borderRadius: "5px" }}>NEW EPISODE</button> : null
                                        }
